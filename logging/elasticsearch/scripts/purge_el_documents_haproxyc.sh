...
user=elastic
pass=""
LOG="/var/log/el_purge/purge_haproxy-${dia}-${mes}-${anio}.log"

echo "START  ${anio}-${mes}-${dia} ${hora}:${min}:${seg}" >> ${LOG}

curl -k -s -XPOST -u ${user}:${pass} 'https://172.29.98.182:9200/haproxy/_delete_by_query?conflicts=proceed' -H 'Content-Type: application/json' -d'{"query": {"range": {"@timestamp": {"lt": "now-2M" }}}}'  > /tmp/output.txt 2>&1
if [ $? -eq 0 ]; then
